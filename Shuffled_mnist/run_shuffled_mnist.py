...
from shuffled_mnist.EOWM import EOWM
...
def get_layer(shape, alpha=0, beta=0, zeros=None):
    ...
    return w, EOWM(shape, alpha, beta)
...
for t_id in range(Task_num):
    ...
    for epoch in range(num_epochs):
        for cur_batch_for_epoch in range(0, len(train_data_size), batch_size):
            ...
            with torch.no_grad():
                layer1.EOWM_learn(w1, inputs, learning_rate, args.alpha, args.beta, lambda1, lambda2, args.mode1)
                layer_out.EOWM_learn(wo, out, learning_rate, args.alpha, args.beta, lambda1, lambda2, args.mode2)
